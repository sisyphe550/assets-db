...
**路由**：`/admin/users`（role=1）、院级管理员无此菜单

**API**：
- 列表：`GET /user/users?page=1&pageSize=20&keyword=`
- 详情：`GET /user/users/:id`
- 创建：`POST /user/users`
- 状态：`PUT /user/users/:id/status`
- 强制下线：`POST /user/users/:id/force-logout`

**页面组成**：ProTable 用户列表 + [+ 创建用户] Modal + 行内启用/禁用开关 + 强制下线按钮

**院级差异**：role=2 调用列表 API 时自动限制为本院子树用户；创建用户时 roleLevel 只能选 3
